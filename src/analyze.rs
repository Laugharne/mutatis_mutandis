...
pub fn pass1(g: &Globals, src_dir: &str, files: Vec<SourceCode>) -> Vec<SourceCode> {
	...
	let input_src_path:  &Path = Path::new("src/main_original.rs");
	...
	for file in &files {
		...
		let ast: File = pass1_parse_ast(ast, nn_entry_point);
...
fn pass1_parse_ast(mut ast: File, mut nn_entry_point: IndexEntryPoint) -> File {
...
fn pass1_parse_function(func: &mut syn::ItemFn, mut nn_entry_point: IndexEntryPoint) {
...
fn pass1_parse_stmt(statement: &mut syn::Stmt, mut nn_entry_point: IndexEntryPoint) {
	if let syn::Stmt::Expr(expr) | syn::Stmt::Semi(expr, _) = statement {
		pass1_parse_expr(expr, nn_entry_point);
	}
}

fn pass1_parse_expr(expr: &mut syn::Expr, mut nn_entry_point: IndexEntryPoint) {
	match expr {
...
fn pass1_parse_condition(cond: &mut syn::Expr, mut nn_entry_point: IndexEntryPoint) {
...
fn pass1_parse_boolean_literal(expr: &mut syn::Expr, mut nn_entry_point: IndexEntryPoint) {
